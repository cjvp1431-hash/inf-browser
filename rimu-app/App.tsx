...
const TAB_SCREENS = [
  ...
  { name: 'WhatsApp',  component: WhatsAppScreen,  label: 'RIMU Bot',  icon: 'chatbubble-ellipses' },
      ...
      screenOptions={({ route }) => {
        ...
        return {
          ...
          tabBarIcon: ({ color, focused }) => {
            const base = screen?.icon ?? 'help';
            const iconName = focused ? base : `${base}-outline`;
            return <Ionicons name={iconName as any} size={focused ? 24 : 22} color={color} />;
          },
        };
      }}
    >
  ...
  if (error) {
    return (
      <View style={styles.loader}>
        <Text style={styles.errorText}>Error al inicializar:{'\n'}{error}</Text>
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  errorText: { color: COLORS.error, textAlign: 'center', padding: 20, lineHeight: 22 },
});

...
type Tab = "dashboard" | "ingreso" | "distribucion" | "cuentas" | "deudas" | "metas" | "logros" | "advisor";
...
function AppInner() {
  ...
  const allNavItems = [...NAV_ITEMS, ...MORE_ITEMS];
  const activeInfo = allNavItems.find((n) => n.id === activeTab);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontSize: 11, color: "#aaa", textTransform: "uppercase", letterSpacing: 1 }}>WealthOS</div>
          ...
          <button
            ...
            style={{
              background: showMore ? "#1a1a2e" : "#e9ecef",
              border: "none",
              borderRadius: "50%",
              width: 38,
              height: 38,
            ...
            {MORE_ITEMS.map((item) => (
              <button
                ...
                style={{
                  width: "100%",
                  padding: "14px 16px",
                  background: activeTab === item.id ? "#f0f4ff" : "#fff",
                  border: "none",
          ...
          {NAV_ITEMS.map((item) => (
            ...
            >
              <span style={{
                fontSize: 22,
                display: "block",
                filter: activeTab === item.id ? "none" : "grayscale(0.5) opacity(0.6)",
              ...
              <span style={{
                fontSize: 10,
                fontWeight: activeTab === item.id ? 700 : 400,
                color: activeTab === item.id ? "#1a1a2e" : "#aaa",
                letterSpacing: 0.2,
              }}>
                {item.label}
              </span>
              {activeTab === item.id && (
                <div style={{ width: 4, height: 4, borderRadius: "50%", background: "#1a1a2e", marginTop: 1 }} />
              )}
            </button>
          ))}
        </div>
